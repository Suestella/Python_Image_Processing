...
class ImageProcessor:
    ...
    def log_transform(self):
        if self.img_original:
            ...
            self.img_current = self.img_current.point(lambda x: 255 * math.log(1 + x, 10) / math.log(1 + 255, 10))
            self.update_image()

    def exp_transform(self):
        if self.img_original:
            self.img_current = self.img_original.copy().convert('L')
            self.img_current = self.img_current.point(lambda x: 255 * (1 - math.exp(-x / 255)))
            self.update_image()

    def linear_transform(self):
        if self.img_original:
            self.img_current = self.img_original.copy().convert('L')
            self.img_current = self.img_current.point(lambda x: 255 * (x - self.img_current.getextrema()[0]) / (
                        self.img_current.getextrema()[1] - self.img_current.getextrema()[0]))
            self.update_image()
